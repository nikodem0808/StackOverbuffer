...
function OnKeyDown(keyData){
  if (keyData.code == 'Enter') document.getElementById('search').dispatchEvent(new MouseEvent('click'));
}
$.ready.then(()=>{
      ...
      data.json().then((jason)=>{
        ...
        jason.items.forEach((i) => {
          table.innerHTML += `<tr id="${++counter}">
            ...
            <td class="rlink"><a class="link" target="blank" href="https://www.stackoverflow.com/questions/${i.question_id}">Open</a></td>
          </tr>`;
        });
      });
    }).catch((e) => {
